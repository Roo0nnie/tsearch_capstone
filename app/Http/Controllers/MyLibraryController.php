...
class MyLibraryController extends Controller
{
    ...
    public function myAdvise(Request $request)
    {
        ...
        if ($query) {
            $savefiles = MyThesis::with('imrad')->whereHas('imrad', function ($query) {$query->where('action', null); })
            ->where('user_code', $user_code)
            ->whereHas('imrad', function ($q) use ($query) {
                $q->where('department', 'like', '%' . $query . '%')
                    ->orWhere('author', 'like', '%' . $query . '%')
            ...
            $savefiles = MyThesis::where('user_code', $user_code)->whereHas('imrad', function($query) {
        ...
        return view('main_layouts.myadvise', compact('querySuggestions','savefiles', 'announcements', 'noAnnouncements', 'admins', 'query'));
    }
